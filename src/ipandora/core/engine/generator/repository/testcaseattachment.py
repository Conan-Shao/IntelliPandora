...
"""
@Author: Shao Feng
@File  : testcaseattachment.py
@Time  : 2024-06-02
"""
from typing import List, Optional
from ipandora.core.base.repository.baserepository import BaseRepository
from ipandora.core.engine.generator.model.data.testcase import TestCaseAttachment
from ipandora.utils.log import logger


class TestCaseAttachmentRepository(BaseRepository):

    def get_attachments_by_test_case_id(self, test_case_id: int) -> List[TestCaseAttachment]:
        query = """
        ...
        return [TestCaseAttachment(**row) for row in rows] if rows else []

    def insert_attachment(self, attachment: TestCaseAttachment) -> int:
    ...
    def update_attachment(self, attachment: TestCaseAttachment) -> int:
        query, values = self.generate_update_query(attachment, "TestCaseAttachments",
                                                   "AttachmentID")
        return self.execute_update(query, tuple(values))

    def insert_attachment_as_transaction(self, attachment: TestCaseAttachment,
